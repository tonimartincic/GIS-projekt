...
def main(database_name):
    ...
    for i in range(0, len(x), 4):
        if not(((x[i] > x[i+1]) and (x[i+1] > x[i+2]) and (x[i+2] > x[i+3])) or (x[i] < x[i+1] and x[i+1] < x[i+2] and x[i+2] < x[i+3])):
            counter_losih += 1
            continue

        if not (((y[i] > y[i + 1]) and (y[i + 1] > y[i + 2]) and (y[i + 2] > y[i + 3])) or (y[i] < y[i + 1] and y[i + 1] < y[i + 2] and y[i + 2] < y[i + 3])):
            counter_losih += 1
            continue
        counter_dobrih += 1
